...
if(isset($_SESSION['adm_user_id'])){
	...
	mysqli_query($link, "TRUNCATE TABLE $t_stats_accepted_languages_per_month") or die(mysqli_error($link));
	...
	if(isset($configGenerateTestDataSav) && $configGenerateTestDataSav == "1"){
		...
		while($row = mysqli_fetch_row($result)) {
			...
			(stats_accepted_language_id, stats_accepted_language_year, stats_accepted_language_month, stats_accepted_language_name, stats_accepted_language_unique, stats_accepted_language_hits) 
			VALUES
			(NULL, $year, $month, $inp_iso_mysql, $inp_unique, $inp_hits)") or die(mysqli_error($link));
		}
