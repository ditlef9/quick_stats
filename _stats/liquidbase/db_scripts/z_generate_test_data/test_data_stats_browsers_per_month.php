...
if(isset($_SESSION['adm_user_id'])){
	...
	mysqli_query($link, "TRUNCATE TABLE $t_stats_browsers_per_month") or die(mysqli_error($link));
	...
	if(isset($configGenerateTestDataSav) && $configGenerateTestDataSav == "1"){
		...
		for($x=0;$x<5;$x++){
			...
			VALUES
			(NULL, $month, $year, $inp_name_mysql, $inp_unique, $inp_hits)") or die(mysqli_error($link));
		}
