...
if(isset($_SESSION['adm_user_id'])){
	...
	mysqli_query($link, "TRUNCATE TABLE $t_stats_languages_per_month") or die(mysqli_error($link));
	...
	if(isset($configGenerateTestDataSav) && $configGenerateTestDataSav == "1"){
		...
		while($row = mysqli_fetch_row($result)) {
			...
			(stats_language_id, stats_language_year, stats_language_month, stats_language_name, stats_language_iso_two, stats_language_flag_path_16x16, 
			stats_language_flag_16x16, stats_language_unique, stats_language_hits) 
			VALUES
			(NULL, $year, $month, $inp_name_mysql, $inp_iso_mysql, $inp_path_mysql, 
