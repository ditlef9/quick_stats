...
if(isset($_SESSION['adm_user_id'])){
	...
	mysqli_query($link, "TRUNCATE TABLE $t_stats_visists_per_month") or die(mysqli_error($link));
	...
	if(isset($configGenerateTestDataSav) && $configGenerateTestDataSav == "1"){
		...
		for($x=$start;$x<$stop;$x++){
			...
			for($y=0;$y<12;$y++){
				...
				if($inp_month < 10){
					$inp_date = "$inp_year-0$inp_month-01";
 				}
				else{
					$inp_date = "$inp_year-$inp_month-01";
				}
				...
				if($inp_year == "$current_year" && $inp_month > $current_month){
					// Out of range
				}
				else{
