...
if(isset($_SESSION['adm_user_id'])){
	...
	mysqli_query($link, "TRUNCATE TABLE $t_stats_os_per_month") or die(mysqli_error($link));
	...
	if(isset($configGenerateTestDataSav) && $configGenerateTestDataSav == "1"){
		...
		foreach($desktop_os as $os){
			...
			VALUES
			(NULL, $month, $year, $inp_name_mysql, $inp_type_mysql, $inp_unique, $inp_hits)") or die(mysqli_error($link));
		}
